...
class StringFileResolver(
    val getString: suspend (String) -> String
)


class SharedDataSource(
    private val res: StringFileResolver,
    private val featureManager: FeatureManager,
    private val metaMagicManager: MetaMagicManager,
    private val featManager: FeatManager,
    private val abilitiesManager: AbilitiesManager,
    ...
    private val subraceManager: SubraceManager,
    ...
    suspend fun generateItemProficiencies() {
        val rootJson = JSONObject(
            res.getString("item_proficiencies.json")
        )
    ...
    suspend fun generateFightingStyles() {
        val dataAsString =
            res.getString("fighting_styles.json")
    ...
    suspend fun generateMetaMagic() {
        val metamagic = mutableListOf<Metamagic>()
        val dataAsString =
            res.getString("metamagic.json")
        ...
        metaMagicManager.postAll(metamagic)
    ...
    suspend fun generateManeuvers() {
        val dataAsString =
            res.getString("maneuvers.json")
        ...
        featureManager.postManeuvers(maneuvers)


        maneuvers.forEach {
            featureManager.insertFeature(it)
        }

        featureManager.insertIndexRef(
            maneuvers.map { it.featureId },
            "Maneuvers"
        )

    }

    suspend fun generateInvocations() {
        val dataAsString =
            res.getString("invocations.json")
        ...
        featureManager.postInvocations(invocations)
    ...
    suspend fun generateInfusions() {
        val dataAsString =
            res.getString("infusions.json")
        ...
        featureManager.postInfusions(infusions)
        val ids = mutableListOf<Int>()
    ...
    private suspend fun extractInfusion(infusionJson: JSONObject): Infusion {
        ...
        choice?.let { entity ->
            ...
            featureManager.insertFeatureChoice(entity)

            featureManager.insertFeatureChoiceIndexCrossRef(
                entity.id,
                "Replicate magic item"
            )

            featureManager.insertFeatureOptionsCrossRef(
                infusionJson.getInt("id"),
                entity.id
            )
        }
    ...
    private fun getItemsByIndex(index: String): List<ItemInterface>? {
        ...
        when (index) {
            "simple_melee_weapons" -> {
                return itemManager.getSimpleWeapons().filter { it.range == "5ft" }
            }
    ...
    private fun getWeaponsByIndex(index: String): List<Weapon>? {
        return when (index) {
            "simple_weapons" -> itemManager.getSimpleWeapons()

            "martial_weapons" -> itemManager.getMartialWeapons()

            "all_weapons" -> {
                val weapons = itemManager.getMartialWeapons().toMutableList()
                itemManager.getSimpleWeapons().let { weapons.addAll(it) }
                weapons
            }
                ...
                itemManager.getMartialWeapons().let { weapons.addAll(it) }
                itemManager.getSimpleWeapons().let { weapons.addAll(it) }
                for (item in weapons) {
    ...
    private fun getLanguagesByIndex(index: String): List<Language>? {
        return when (index) {
            "all_languages" -> languageManager.getAllLanguages()
            else -> null
        }
    }
    ...
    private fun getArmorByIndex(index: String): List<Armor>? {
        return when (index) {
            "all_armor" -> itemManager.getArmors()
            else -> {
                var result: List<Armor>? = null
                for (item in itemManager.getArmors()) {
                    if (item.name?.lowercase() == index.lowercase()) {
                        result = listOf(item)
                        break
                    }
    ...
    private fun getSpellIdByName(name: String): Int {
        return spellManager.getSpellIdByName(name)
    }
    ...
    suspend fun generateFeats() {
        val dataAsString =
            res.getString("feats.json")
        ...
        for (featIndex in 0 until featsJson.length()) {
            ...
            feat.id = featIndex + 1
            featManager.insertFeat(
                feat
            )

            features.forEach {
                featManager.insertFeatFeatureCrossRef(
                    featIndex + 1,
                    it
                )
            }
    ...
    suspend fun generateItems() {
        itemManager.postMartialWeapons(
            generateWeapons(
                res.getString("martial_weapons.json"),
                true
            )
        )

        itemManager.postSimpleWeapons(
            generateWeapons(
                res.getString("simple_weapons.json"),
                false
            )
        )

        itemManager.postArmors(generateArmor())

        itemManager.postMisc(generateMiscItems())
    }


    private suspend fun generateMiscItems(): List<ItemInterface> {
        val dataAsString =
            res.getString("misc_items.json")
        val result = mutableListOf<ItemInterface>()
        ...
        for (index in 0 until itemsJson.length()) {
            ...
            try {
                ...
                logManager.logError("generateMiscItems: $itemJson")
                throw e
            }
        }
    ...
    private suspend fun generateArmor(): List<Armor> {
        val dataAsString =
            res.getString("armor.json")
    ...
    suspend fun generateSkills() {
        ...
        abilitiesManager.postAbilitiesToSkills(abilitiesToSkills)
    ...
    suspend fun updateSpells() {
        val dataAsString =
            res.getString("spells.json")
        ...
        for (spellIndex in 0 until spellsJson.length()) {
            ...
            val range = try {
                spellJson.getString("range")
            } catch(e: JSONException) {
                "${spellJson.getInt("range")} feet"
            }
            ...
            featureManager.insertFeatureSpellCrossRef(
                id,
                id
            )

            classes.forEach {
                classManager.insertClassSpellCrossRef(
                    classNameToId[it.lowercase()]!!,
                    id
                )
            }
    ...
    suspend fun generateLanguages() {
        val dataAsString =
            res.getString("languages.json")
    ...
    suspend fun updateBackgrounds() {
        val dataAsString =
            res.getString("backgrounds.json")
        ...
        for (backgroundIndex in 0 until backgroundsJson.length()) {
                        ...
                        for (langChoiceIndex in 0 until languageChoicesJson.length()) {
                            ...
                            try {
                                ...
                                if (index == "all_languages") { //If we add more language indexes this may need to be moved into a function
                                    languageManager.getAllLanguages().let { from.addAll(it) }
                                }
                            } catch (e: JSONException) {
            ...
            try {
                ...
                for (index in 0 until backgroundSpellsJson.length()) {
                    ...
                    backgroundManager.insertBackgroundSpellCrossRef(
                        backgroundIndex + 1,
                        id
                    )
                }
            ...
            backgroundManager.insertBackground(
                BackgroundEntity(
                    name = name,
                    desc = desc,
                    spells = emptyList(),
            ...
            features.forEach {
                backgroundManager.insertBackgroundFeatureCrossRef(
                    backgroundIndex + 1,
                    it
                )
            }
    ...
    suspend fun updateRaces() {
        val dataAsString =
            res.getString("races.json")
        ...
        for (raceIndex in 0 until racesJson.length()) {
            ...
            subraceIds.forEach {
                raceManager.insertRaceSubraceCrossRef(
                    it,
                    raceIndex + 1
                )
            }
            ...
            traits.forEach {
                raceManager.insertRaceFeatureCrossRef(
                    raceIndex + 1,
                    it
                )
            }
    ...
    private suspend fun extractSubraces(jsonArray: JSONArray): List<Int> {
        ...
        for (index in 0 until jsonArray.length()) {
            ...
            try {
                ...
                subraceManager.insertSubraceFeatureCrossRef(
                    id,
                    it
                )
            }
            ...
            featChoices?.forEach {
                subraceManager.insertSubraceFeatChoiceCrossRef(
                    it,
                    id
                )
            }
    ...
    private fun extractFeatChoices(jsonArray: JSONArray): List<Int> {
        ...
        for (index in 0 until jsonArray.length()) {
            ...
            featManager.insertFeatChoice(featChoice)
    ...
    private fun getProficienciesByIndex(index: String): List<Proficiency> {
        ...
        when (index) {
            ...
            "skill_proficiencies" -> {
                abilitiesManager.getAbilitiesToSkills().values.forEach {
                    it.forEach { item ->
    ...
    suspend fun updateClasses() {
        ...
        for (classIndex in 0 until classesJson.length()) {
            ...
            for (subclassIndex in 0 until subclassesJson.length()) {
                ...
                subclassManager.insertSubclass(
                    SubclassEntity(
                ...
                classManager.insertClassSubclassId(
                    classIndex + 1,
                    id
                )

                extractFeatures(subclassJson.getJSONArray("features")).forEach {
                    subclassManager.insertSubclassFeatureCrossRef(
                        it,
                        id
                    )
                }
                ...
                try {
                    ...
                    for (index in 0 until subclassSpellJson.length()) {
                        ...
                        //TODO check that this functions
                        subclassManager.insertSubclassSpellCrossRef(
                            id,
                            spellManager.getSpellIdByName(spellJson.getString("name"))
                        )
                    }
                } catch (_: JSONException) {
                }
            ...
            featureIds.forEach {
                classManager.insertClassFeatureCrossRef(
                    classIndex + 1,
                    it
                )
            }
                ...
                try {
                    when (val index = featureJson.getString("index")) {
                        ...
                        "Draconic Ancestry" -> {
                            ...
                            parentChoiceId?.let {
                                featureManager.insertOptionsFeatureCrossRef(
                                     featureManager.getFeatureIdByName(index),
                                     it
                                )
                            }
                        }
                    ...
                    suspend fun extractAndAddChoice(choiceJson: JSONObject) {
                        ...
                        if (choose != Choose(0) && choiceId != null) {
                            ...
                            featureManager.insertFeatureChoice(
                                entity
                            )
                    ...
                    featureManager.insertFeature(feature)
