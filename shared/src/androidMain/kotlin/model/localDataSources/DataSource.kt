...
class LocalDataSourceImpl constructor(
    ...
    private suspend fun generateItemProficiencies() {
        val rootJson = JSONObject(
            Res.readBytes("files/raw/item_proficiencies.json").decodeToString()
        )
    ...
    private suspend fun generateFightingStyles() {
        val dataAsString =
            Res.readBytes("files/raw/fighting_styles.json").decodeToString()
    ...
    private suspend fun generateMetaMagic() {
        val metamagic = mutableListOf<Metamagic>()
        val dataAsString =
            Res.readBytes("files/raw/metamagic.json").decodeToString()
    ...
    private suspend fun generateManeuvers() {
        val dataAsString =
            Res.readBytes("files/raw/maneuvers.json").decodeToString()
    ...
    private suspend fun generateInvocations() {
        val dataAsString =
            Res.readBytes("files/raw/invocations.json").decodeToString()
    ...
    private suspend fun generateInfusions() {
        val dataAsString =
            Res.readBytes("files/raw/infusions.json").decodeToString()
    ...
    private suspend fun generateFeats() {
        val dataAsString =
            Res.readBytes("files/raw/feats.json").decodeToString()
    ...
    private suspend fun generateItems() {
        ...
        _martialWeapons.postValue(
            generateWeapons(
                Res.readBytes("files/raw/martial_weapons.json").decodeToString(),
                true
            )
        )
        ...
        _simpleWeapons.postValue(
            generateWeapons(
                Res.readBytes("files/raw/simple_weapons.json").decodeToString(),
                false
            )
        )
        ...
        _armors.postValue(generateArmor())

        _items.addSource(_miscItems) { value -> addData(value) }
        _miscItems.postValue(generateMiscItems())
    ...
    private suspend fun generateMiscItems(): List<ItemInterface> {
        val dataAsString =
            Res.readBytes("files/raw/misc_items.json").decodeToString()
        val result = mutableListOf<ItemInterface>()
    ...
    private suspend fun generateArmor(): List<Armor> {
        val dataAsString =
            Res.readBytes("files/raw/armor.json").decodeToString()
    ...
    private suspend fun generateSkills() {
        val dataAsString =
            Res.readBytes("files/raw/skills.json").decodeToString()
    ...
    private suspend fun updateSpells() {
        val dataAsString =
            Res.readBytes("files/raw/spells.json").decodeToString()
    ...
    private suspend fun generateLanguages() {
        val dataAsString =
            Res.readBytes("files/raw/languages.json").decodeToString()
    ...
    private suspend fun updateBackgrounds() {
        val dataAsString =
            Res.readBytes("files/raw/backgrounds.json").decodeToString()
    ...
    private suspend fun updateRaces() {
        val dataAsString =
            Res.readBytes("files/raw/races.json").decodeToString()
    ...
    private suspend fun updateClasses() {
        val dataAsString =
            Res.readBytes("files/raw/classes.json").decodeToString()
                    ...
                    for (index in 0 until subclassSpellJson.length()) {
                        ...
                        val cursor = db.query(
                            "SELECT id FROM spells WHERE LOWER(spells.name) LIKE LOWER(\'${
                                spellJson.getString("name").replace("'", "_")
                            }\')",
                            arrayOf()
                        )
