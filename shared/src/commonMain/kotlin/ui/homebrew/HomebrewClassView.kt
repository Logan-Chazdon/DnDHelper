...
fun HomebrewClassView(
    ...
    val clazz = viewModel.clazz?.collectAsState()
                                ...
                                item {
                                    Row {
                                        OutlinedTextField(
                                            ...
                                            onValueChange = {
                                                val value =
                                                    viewModel.spellCastingSpellsAndCantripsKnown.values.elementAt(index)
                                                val key =
                                                    viewModel.spellCastingSpellsAndCantripsKnown.keys.elementAt(index)
                                        ...
                                        if (viewModel.spellCastingLearnsSpells.value) {
                                            OutlinedTextField(
                                                value = viewModel.spellCastingSpellsAndCantripsKnown.values.elementAt(
                                                    index
                                                ).second,
                                                onValueChange = {
                                                    val old =
                                                        viewModel.spellCastingSpellsAndCantripsKnown.values.elementAt(
                                                            index
                                                        )
                                                    val key =
                                                        viewModel.spellCastingSpellsAndCantripsKnown.keys.elementAt(
                                                            index
                                                        )
                                                    viewModel.spellCastingSpellsAndCantripsKnown[key] =
                                                        old.copy(second = it)
                                                },
                                                modifier = Modifier.weight(1f, true),
                                                label = {
                        ...
                        Column {
                            ...
                            GenericSelectionView(
                                onClick = {
                                    var id = 0
                                    scope.launch {
                                        id =  viewModel.subclasses!!.lastOrNull()!![it].subclassId
                                    }.invokeOnCompletion {
                                        navController.navigate("homebrewView/homebrewSubclassView/${id}")
                                    }
                                },
                                chosen = viewModel.subclasses?.collectAsState(listOf())?.value.let {
