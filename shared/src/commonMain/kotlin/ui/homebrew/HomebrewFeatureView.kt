...
fun HomebrewFeatureView(
    ...
    GenericSelectionPopupView(
        ...
        items = viewModel.allSpells.collectAsState(emptyList()).value,
        detailsView = {
    ...
    GenericSelectionPopupView(
        ...
        items = viewModel.allInfusions.collectAsState(emptyList()).value,
        detailsView = null,
        getName = {
            it.name
        },
    ...
    GenericSelectionPopupView(
        ...
        items = viewModel.allLanguages.collectAsState(emptyList()).value,
        detailsView = null,
        getName = {
            it.name ?: ""
        },
        ...
        item {
            Card(
                ...
                Column(
                    ...
                    val featureChoices = viewModel.featureChoices.collectAsState(listOf()).value
                    AttributeView(
                                ...
                                items(featureChoices) { choice ->
                                    FeatureChoiceView(
                                        ...
                                        onNew = {
                                            var id = 0
                                            scope.launch {
                                                id = viewModel.createDefaultFeature(choice.id)
                                            }.invokeOnCompletion {
                                        ...
                                        indexes = viewModel.featureIndexes.collectAsState(listOf()).value,
...
class FeatureChoiceIndexCrossRef(
    val choiceId: Int,
    val index: String,
...
private fun ProficiencySelectionView(
    ...
    fromList: Flow<Map<String, List<String>>>,
) {
    val expanded = remember {
        mutableStateOf(false)
    }
    ...
    GenericSelectionPopupView(
        ...
        items = fromList.collectAsState(mapOf()).value.flatMap { it.value },
        detailsView = null,
        getName = {
            it
        },
