...
class MultipleChoiceDropdownStateImpl : MultipleChoiceDropdownState {
    ...
    private val _selectedNames = MutableStateFlow("")
    override val selectedNames: MutableStateFlow<String> = _selectedNames
    ...
    fun <T> getSelected(from : List<T>) : List<T>{
        val returnList = mutableListOf<T>()

        if(from.isNotEmpty()) {
            for (i in from.indices) {
                for (x in 0 until selectedList.getOrElse(i, { 0 })) {
